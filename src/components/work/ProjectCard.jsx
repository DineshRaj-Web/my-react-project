...
const SectionItem = React.memo(({ title, content, color, iconColor }) => (
    <div className="group/item relative">
        <div className="absolute -left-4 top-0 bottom-0 w-[1px] bg-white/5 group-hover/item:bg-cyan-500/30 transition-colors duration-500" />
        <h4 className={`${color}/80 text-[0.6rem] uppercase font-black tracking-[0.3em] mb-3 md:mb-4 flex items-center gap-2 group-hover/item:tracking-[0.4em] transition-all duration-500`}>
            <span className={`w-1.5 h-[1px] ${iconColor} shadow-[0_0_8px_rgba(6,182,212,0.5)]`} />
            {title}
        </h4>
...
const ProjectCard = ({ project, index }) => {
    ...
    return (
        <motion.div
            ...
            className="group relative bg-white/[0.02] backdrop-blur-md border border-white/10 rounded-[2rem] md:rounded-[3rem] overflow-hidden transition-all duration-1000 flex flex-col hover:border-white/20 min-h-[450px] md:min-h-[500px]"
        >
            {/* Cyber-Chrome Metallic Base */}
            <div className="absolute inset-0 -z-20">
                <div className="absolute inset-0 bg-gradient-to-br from-white/[0.04] via-transparent to-transparent opacity-50 group-hover:opacity-100 transition-opacity duration-1000" />
                <div className="absolute inset-0 opacity-[0.02] bg-[url('https://www.transparenttextures.com/patterns/brushed-alum.png')]" />
                <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
            </div>

            {/* Liquid Reflective Spotlight */}
            <motion.div
                className="pointer-events-none absolute -inset-px rounded-[2rem] md:rounded-[3rem] opacity-0 transition-opacity duration-1000 group-hover:opacity-100"
                style={{
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 overflow-y-auto pr-4 custom-scrollbar px-1">
                            {project.institutions.map((inst, i) => (
                                <motion.a
                                    ...
                                    whileTap={{ scale: 0.98 }}
                                    className="group/inst-link relative h-14 bg-white/[0.03] border border-white/5 rounded-2xl flex items-center justify-between px-6 hover:bg-cyan-500/10 hover:border-cyan-500/30 transition-all duration-500"
                                >
                ...
                <div className="flex justify-between items-start mb-8 md:mb-14">
                    <div className="flex-1">
                        <div className="flex items-center gap-4 mb-4 md:mb-6">
                            <motion.div
                                className={`h-1.5 w-1.5 rounded-full bg-gradient-to-r ${project.color} shadow-[0_0_15px_rgba(6,182,212,0.5)]`}
                        ...
                        <h3 className="text-3xl md:text-5xl lg:text-6xl font-black text-white leading-[0.9] tracking-tighter mb-3 md:mb-4 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-white/40 transition-all duration-1000">
                    ...
                    <motion.button
                        ...
                        whileTap={{ scale: 0.9 }}
                        className="relative w-16 h-16 md:w-24 md:h-24 rounded-full border border-white/5 flex items-center justify-center bg-white/[0.02] hover:bg-white/[0.08] hover:border-cyan-500/30 transition-all duration-700 shrink-0 ml-2 md:ml-8 group/btn"
                    >
                ...
                <div className="flex flex-col gap-8 md:gap-12 mb-8 md:mb-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-12">
                        ...
                        <div className="relative h-full">
                            <div className="h-full glass-panel rounded-[2.5rem] p-8 lg:p-10 group/impact hover:border-emerald-500/30 transition-all duration-1000 relative bg-white/[0.02] border border-white/10">
                                ...
                                <div className="space-y-6">
                                    {projectData.outcomePoints.map((point, i) => (
                                        <div key={i} className="flex items-start gap-4 group/point transition-all duration-300 hover:translate-x-1">
                                            <div className="mt-1 flex-shrink-0 w-5 h-5 rounded-md bg-white/5 border border-white/10 flex items-center justify-center text-[0.8rem] text-emerald-500/70 group-hover/point:text-emerald-400 transition-all font-black">
                                                »
                                            </div>
