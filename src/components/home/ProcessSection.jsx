...
function ProcessCard({ step, index }) {
    ...
    return (
        <motion.div
            ...
            whileTap={{ scale: 0.98 }}
            className={`relative grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 mb-32 last:mb-0 group cursor-default`}
        >
            {/* HUD Content Side */}
...
export default function ProcessSection() {
    ...
    return (
        <section ref={ref} className="py-24 bg-black relative overflow-hidden text-white">
            ...
            <div className="max-w-7xl mx-auto px-6 relative">
                ...
                >
                    ...
                    <p className="text-gray-400 text-lg md:text-xl max-w-2xl leading-relaxed font-light">
                        A rigorous architectural approach to <span className="text-white font-medium italic underline decoration-purple-500/50">deconstructing chaos</span> and deploying scalable digital clarity.
                    </p>
                </motion.div>

                {/* Technical Central Rail */}
                <div className="absolute left-1/2 top-[400px] bottom-0 w-px bg-white/5 -translate-x-1/2 hidden md:block">
                    <motion.div
                        style={{ height: useTransform(pathLength, val => `${val * 100}%`) }}
                        className="w-full bg-gradient-to-b from-cyan-400 via-purple-500 to-emerald-400 shadow-[0_0_15px_rgba(168,85,247,0.5)]"
                    />
                    {/* Scanning Tracer */}
                    <motion.div
                        animate={{ top: ['0%', '100%'] }}
                        transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
                        className="absolute left-1/2 -translate-x-1/2 w-1.5 h-16 bg-gradient-to-b from-transparent via-cyan-400 to-transparent blur-[2px]"
                    />
                </div>
